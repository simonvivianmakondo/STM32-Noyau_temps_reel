...
#ifdef EMU
    #include "hwsupport/uart.h"       // déclare cmsdk_usart_init
#else
    #include "hwsupport/stm_uart.h"   // déclare stm_usart_init
#endif
...
{
	#ifdef EMU
		cmsdk_usart_init(115200);   // UART émulé CMSDK
	#else
		stm_usart_init(115200);     // UART réel STM32H7
	#endif
