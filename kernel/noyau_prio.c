 ...
 */
 void start(TACHE_ADR adr_tache) {
    ...
    /* Partie 6 : changer cette valeur ! */
    _tache_c = MAX_TACHES_NOYAU;
    // Il s’agit maintenant de la tâche
    //de priorité 7 et d’index 0, soit 111000 = 56.
 ...
 */
 ...
 {
	/* Partie 6 : identifiant de la tâche */
	uint16_t id;
    /* pointeur d'une case de _noyau_tcb         */
    NOYAU_TCB *p;

    /* Partie 6. Utiliser un tableau d'identifiants de tâches, un par file de priorité */
    //static int tache = -1;
    static uint8_t next_index[MAX_PRIO] = {0};

    /* Q2.14: debut section critique */
    _lock_();            

    /* Partie 6. Allouer un identifiant à la tâche. */
    /* Cet identifiant est composé du numéro de tâche dans la file (3 bits de poids faible)
     * suivis de la priorité de la tâche (3 bits suivants). Si le numéro
     * de la tâche dans la file est plus grand que MAX_TACHES, sortir du noyau.
     */
    ...
    uint16_t index = next_index[prio]++;
    ...
    /* Partie 6. Remplacer par le calcul de l'identifiant de tâche */
    id = (prio << 3) | index;
   ...
   /* creation du contexte de la nouvelle tache */
    ...
    /* Partie 5. Initialisation du compteur de délai à zéro */

    /* Partie 6. Sauvegarder le paramètre de la tâche dans le tcb */
 ...
 */
 void active(uint16_t tache)
 {
    ...
    if (p->status == CREE) {                    /* n'active que si receptif  */
        ...
        c->r0 = (uint32_t) p->arg;
        /* Partie 6. Affecter le pointeur d'argument */
        										/* Argument de la tâche */
 ...
 *--------------------------------------------------------------------------*/
 ...
 {
     NOYAU_TCB *p;
     char sep;							  /* Séparateur pour le chronogramme */
     ...
     if (_timer_event) {
     	/* Dans le cas où la préemption est dûe au timer : maj des délais */
     	delay_process();
     	sep = '|';
     } else {
     	sep = ' ';
     ...
     /* Partie 6. Affichage des chronogrammes */
     /* Attention : augmente considérablement le temps d'administration */
     if (_tache_c == MAX_TACHES_NOYAU) {
...
void schedule(void)
{
	...
	/* Lire le numéro d'exception depuis le registre ipsr */
	__asm__ __volatile__(
			"mrs %[result], ipsr\n\t"
			: [result] "=r" (isr_num));
	...
	if (isr_num == 15) {
		/* Flag global indiquant que la préemption est dûe à l'horloge */
		_timer_event = 1;
	}

	/* Planifier une execption pend_svc */
    pendsv_trigger();
}
...
/*
 * recupere le tcb d'une tâche
 * entre  : numéro de tâche
 * sortie : pointeur vers le tcb de la tâche
 * description : fonction d'acces a la valeur d'identite de la tache courante
 */
NOYAU_TCB* noyau_get_p_tcb(uint16_t t){
	return(&_noyau_tcb[t]);
}
